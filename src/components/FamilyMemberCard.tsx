...
const FamilyMemberCard: React.FC<FamilyMemberCardProps> = ({
  ...
  const isCurrentUser = member.userId === user?.uid || member.isCurrentUser;
  ...
  return (
    <div
      ...
      className={`
        ...
        ${isCurrentUser ? 'border-2 border-[#ff853f]' : ''}
      `}
      style={{
        width: '320px',
    ...
    >
      <div className="p-4">
        {isCurrentUser && (
          <div className="absolute -top-2 left-1/2 transform -translate-x-1/2 bg-[#ff853f] text-white px-2 py-0.5 rounded-full text-xs font-medium">
            This is you
          </div>
        )}
